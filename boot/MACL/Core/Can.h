...
#endif
/* Check if current file and CAN configuration header file are of the same software version */
#if ((CAN_SW_MAJOR_VERSION != COMTYPE_SW_MAJOR_VERSION) || \
     (CAN_SW_MINOR_VERSION != COMTYPE_SW_MINOR_VERSION) || \
     (CAN_SW_PATCH_VERSION != COMTYPE_SW_PATCH_VERSION))
...
typedef struct
    {
...
#else
#if (CAN_MCAN_SPECIFIC == STD_ON)
        CONST(uint8, CAN_CONST) u8MaxMbRxCount; /**< @brief Max Message Buffer number for Rx Fifo and Buffer. */
        ...
        CONST(uint32, CAN_CONST) u32CanWakeupSourceID; /*< @brief WakeUp source ID (type def uint32 EcuM_WakeupSourceType - in EcuM.h (uint32) ). */
        ...
        CONST(Can_PCallBackType, CAN_CONST) Can_ErrorNotification; /*< @brief Pointer to Error interrupt notification function (ESR[ERR_INT]). */
#endif
        CONST(uint32, CAN_CONST) u32Options; /*< @brief BusOff Sw Recovery, RXFifo En, IDAM Type,  Event Trigger Mode TxProcessing/RxProcessing/BusoffProcessing/WakeuProcessing: Polling vs Interrupt mode. */
        ...
        CONST(Can_HwHandleType, CAN_CONST) uFirstHrhIndex;
...
/* Extern declarations for CanController Configuration and CanConfigType. Exports the PC or PB Can_ConfigType structures. */
    ...
    /* @violates @ref Can_h_REF_5 Violates MISRA 2004 Required Rule 8.7, Required*/
