...
/* Check if current file and ECUM_CBK header file are of the same Software version */
    #if ((CAN_AR_RELEASE_MAJOR_VERSION_C != ECUM_CBK_AR_RELEASE_MAJOR_VERSION) || \
        (CAN_AR_RELEASE_MINOR_VERSION_C != ECUM_CBK_AR_MINOR_RELEASE_VERSION) || \
            ...
            */
            ...
            else
            {
                ...
                Can_pControlerDescriptors = Config->ControlerDescriptors;
                    ...
                    for ( can_controller = 0U; can_controller < (uint8)Can_pCurrentConfig->u8ControllersConfigured; can_controller++)
                    {
                        ...
                        if ( CAN_NULL_OFFSET != (Can_pControlerDescriptors[can_controller].u8ControllerOffset))
                        {
                            /* Init - ControllerState, uFirstHrhIndex, HthOffset, DefaultConfigIndex, u8MaxMBCount, u32CanWakeupSourceID, Freeze controller and more ... */
                            /*
                            ...
                            */
                            Can_IPW_ChangeBaudrate( can_controller);
                        }
                    }
#if (CAN_DEV_ERROR_DETECT == STD_ON)
                /*
...
FUNC(Std_ReturnType, CAN_CODE) Can_ChangeBaudrate( VAR(uint8, AUTOMATIC) Controller,
                                                    CONST(uint16, AUTOMATIC) Baudrate)
    {
        ...
        else
        {
            ...
            if ( (Controller >= (uint8)Can_pCurrentConfig->u8ControllersConfigured) ||
            /* @violates @ref Can_c_REF_4 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
                 (CAN_NULL_OFFSET == (Can_pControlerDescriptors[Controller].u8ControllerOffset)) )
            {
                /*
...
*/
                        ...
                        else
                        {
                            ...
                            */
                            Can_IPW_ChangeBaudrate(Controller);
                        }
#if (CAN_DEV_ERROR_DETECT == STD_ON)
                    }
            }
        ...
        */
        ...
        else
        {
            ...
            else
            {
                ...
                /* @violates @ref Can_c_REF_4 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
                eReturnValue = Can_IPW_SetControllerMode( Controller, &(Can_pCurrentConfig->ControlerDescriptors[Controller]), Transition,(boolean)TRUE);
#if (CAN_DEV_ERROR_DETECT == STD_ON)
            }
        }
#endif /* (CAN_DEV_ERROR_DETECT==STD_ON)  */
...
FUNC(void, CAN_CODE) Can_DisableControllerInterrupts( VAR(uint8, AUTOMATIC) Controller)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                /* @violates @ref Can_c_REF_4 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
                Can_IPW_DisableControllerInterrupts( Controller, &(Can_pCurrentConfig->ControlerDescriptors[Controller]));
#if (CAN_DEV_ERROR_DETECT == STD_ON)
            }
        }
#endif /* (CAN_DEV_ERROR_DETECT == STD_ON) */
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if ( (PduInfo->length) > CAN_MB_DATABYTES )
                    {
                        /*
                    ...
                    else
                    {
                        ...
                        eReturnValue = Can_IPW_Write( Hth, PduInfo);
#if (CAN_DEV_ERROR_DETECT == STD_ON)
                    }
                }
            }
...
#endif /* (CAN_DEV_ERROR_DETECT == STD_ON) */
        /* Loop through all Can controllers configured based CanControllerId parameter. */
        for ( can_controller = (uint8)0U; can_controller < (uint8)Can_pCurrentConfig->u8ControllersConfigured; can_controller++)
        {
            /*
...
/* @violates @ref Can_c_REF_4 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
            if ( (CAN_NULL_OFFSET !=(Can_pControlerDescriptors[can_controller].u8ControllerOffset))  &&
            /* @violates @ref Can_c_REF_4 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
                 ((CAN_CONTROLLERCONFIG_BOPOL_EN_U32 & (uint32)(Can_pControlerDescriptors[can_controller].u32Options)) != (uint32)0x0U) )
            {
                ...
                if ( (uint8)E_OK == Can_IPW_CheckBusOff(can_controller) )
                {
                    ...
                    /* @violates @ref Can_c_REF_4 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
                    if ( CAN_OK == Can_IPW_SetControllerMode( can_controller,  &(Can_pCurrentConfig->ControlerDescriptors[can_controller]), CAN_T_STOP,(boolean)FALSE) )
                    {
...
FUNC(void, CAN_CODE) Can_MainFunction_Wakeup( void)
{
    ...
    else
    {
        ...
        for ( can_controller = (uint8)0U; can_controller < (uint8)Can_pCurrentConfig->u8ControllersConfigured; can_controller++)
        {
            ...
            */
            if ( (CAN_NULL_OFFSET != (Can_pControlerDescriptors[can_controller].u8ControllerOffset)) &&
                  ((CAN_CONTROLLERCONFIG_WAKSUP_EN_U32 & (uint32)(Can_pControlerDescriptors[can_controller].u32Options)) != (uint32)0x0U) &&
                 ((CAN_CONTROLLERCONFIG_WKPOL_EN_U32 & (uint32)(Can_pControlerDescriptors[can_controller].u32Options)) != (uint32)0x0U) )
            {
                /* Check for wake up condition. */
                if ( (uint8)E_OK == Can_IPW_CheckWakeup( can_controller) )
                {
                    /* Process Wakeup event. */
                   if ( CAN_OK == Can_IPW_SetControllerMode(can_controller, &Can_pCurrentConfig->ControlerDescriptors[can_controller], CAN_T_WAKEUP,(boolean)FALSE) )
                    {
                    /* Process WakeUp event. Clear the wakeup interrupt flag. */
                    EcuM_CheckWakeup( Can_pControlerDescriptors[can_controller].u32CanWakeupSourceID );
        ...
        else
        {
            ...
            */
            ...
            else
            {
                ...
                eReturnValue = CAN_NOT_OK;
#if (CAN_WAKEUP_SUPPORT == STD_ON)
                /* Call IPW function for checking wakeup status. */
                eReturnValue = (Can_ReturnType)Can_IPW_CheckWakeup( controller);
#endif            
...
FUNC(Std_ReturnType, CAN_CODE) Can_SetClockMode( VAR(uint8, AUTOMATIC) can_controller, VAR(Can_ClockMode, AUTOMATIC) can_clk_mode)
    {
        ...
        {
           if ( (can_controller >= (uint8)Can_pCurrentConfig->u8ControllersConfigured) ||
             /* @violates @ref Can_c_REF_4 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
                 (CAN_NULL_OFFSET == (Can_pControlerDescriptors[can_controller].u8ControllerOffset)) )
            {
                /*
                   (CAN210) If development error detection for the Can module is enabled: The function Can_EnableControllerInterrupts shall
                           raise the error CAN_E_PARAM_CONTROLLER if the parameter Controller is out of range.
