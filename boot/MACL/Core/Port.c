...
/**
* @violates @ref PORT_C_REF_5 MISRA 2004 Required Rule 8.10, All declarations and definitions of 
...
)
{
    ...
    {
        Det_ReportError((uint16)PORT_MODULE_ID, PORT_INSTANCE_ID, (uint8)PORT_INIT_ID, (uint8)PORT_E_PARAM_CONFIG);
    }
    else
#endif /* (STD_ON == PORT_DEV_ERROR_DETECT) */
#endif /*  not PORT_PRECOMPILE_SUPPORT */ 
    {
...
)
{
    ...
    /** @violates @ref PORT_C_REF_6 MISRA 2004 Required Rule 17.4, Array indexing shall be the only allowed form of pointer */
...
#if (STD_ON == PORT_DEV_ERROR_DETECT)
        if (PORT_E_MODE_UNCHANGEABLE == u8PinModeError)
        {
            Det_ReportError((uint16)PORT_MODULE_ID, (uint8)PORT_INSTANCE_ID, (uint8)PORT_SETPINMODE_ID, (uint8)PORT_E_MODE_UNCHANGEABLE);
        }
        else if (PORT_E_PARAM_INVALID_MODE == u8PinModeError)
        {
            Det_ReportError((uint16)PORT_MODULE_ID, (uint8)PORT_INSTANCE_ID, (uint8)PORT_SETPINMODE_ID, (uint8)PORT_E_PARAM_INVALID_MODE);
        }
        else
        {
            /* Nothing to do. Else branch required by Misra */
        }
#endif /* (STD_ON == PORT_DEV_ERROR_DETECT) */
    }
}
...
(
    P2VAR(Std_VersionInfoType, PORT_VAR_FAST, PORT_APPL_DATA) versioninfo
)
{
#if (STD_ON == PORT_DEV_ERROR_DETECT)
