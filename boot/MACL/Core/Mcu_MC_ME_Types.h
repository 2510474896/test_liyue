...
==================================================================================================*/
/** @violates @ref Mcu_MC_ME_Types_h_REF_1 MISRA 2004 Required Rule 19.15, Repeated include files.*/
#include "Mcu_Cfg.h"
...
#define MCU_MC_ME_TYPES_AR_RELEASE_MINOR_VERSION      0
#define MCU_MC_ME_TYPES_AR_RELEASE_REVISION_VERSION   3
...
/* Mcu_MC_ME_ModeConfigType structure are valid */
...
typedef struct
{
    ...
    /* all are enabled, they are written starting with CCTL0, if different from 0 */
    VAR( uint16, MCU_VAR) u16CCTL0;   /**< @brief Configuration for ME_CCTL0. */
    VAR( uint16, MCU_VAR) u16CCTL1;   /**< @brief Configuration for ME_CCTL1. */
    VAR( uint16, MCU_VAR) u16CCTL2;   /**< @brief Configuration for ME_CCTL2. */
    VAR( uint16, MCU_VAR) u16CCTL3;   /**< @brief Configuration for ME_CCTL3. */
    VAR( uint16, MCU_VAR) u16CCTL4;   /**< @brief Configuration for ME_CCTL4. */
...
typedef struct
{
    ...
    */
    ...
    /* these are written starting from CADDR0, if different from 0 */
    /**
    * < @brief This registers gives the boot address for z4a and a bit for controlling whether
    * Core0 is to be reset on the next mode change that has Core0 configured to be running in the
    * target mode.
    */
    VAR( uint32, MCU_VAR) u32McMeCore0ResetControl;
    /**
    * < @brief This registers gives the boot address for z4a and a bit for controlling whether
    * Core1 is to be reset on the next mode change that has Core1 configured to be running in the
    * target mode.
    */
    VAR( uint32, MCU_VAR) u32McMeCore1ResetControl;
    /**
    * < @brief This registers gives the boot address for z4a and a bit for controlling whether
    * Core2 is to be reset on the next mode change that has Core2 configured to be running in the
    * target mode.
    */
    VAR( uint32, MCU_VAR) u32McMeCore2ResetControl;
    /**
    * < @brief This registers gives the boot address for z4a and a bit for controlling whether
    * Core3 is to be reset on the next mode change that has Core3 configured to be running in the
    * target mode.
    */
    VAR( uint32, MCU_VAR) u32McMeCore3ResetControl;
    /**
    * < @brief This registers gives the boot address for z4a and a bit for controlling whether
    * Core4 is to be reset on the next mode change that has Core4 configured to be running in the
    * target mode. */
    VAR( uint32, MCU_VAR) u32McMeCore4ResetControl;
#endif /* MCU_CONFIGURE_CADDRN == STD_ON */

} Mcu_MC_ME_ModeConfigType;
